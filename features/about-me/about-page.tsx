...
const achievements = [
  ...
  ["2nd Place", "HackAtom — Science Festival Myanmar 2025", "Russian + Myanmar · Team TenVoltTitans · November 2025"],
  ...
  return <main id="main-content">
    ...
    <motion.section {...sectionMotion} className="section-shell border-t border-borderSoft md:!pt-20" aria-labelledby="journey-title">
      <div className="grid gap-12 lg:grid-cols-[0.72fr_1.28fr] lg:gap-20"><div className="lg:self-start lg:pt-[132px]"><SectionIntro id="journey-title" label="My journey" title="Still learning. Still building." text="My path has grown through curiosity, practical projects, and a constant willingness to learn and try something new." /></div>
        <div className="relative pl-8 sm:pl-12"><div aria-hidden="true" className="absolute bottom-0 left-[5px] top-[22px] w-px sm:top-6 bg-borderMedium sm:left-[9px]" /><motion.div aria-hidden="true" initial={reduce ? false : { scaleY: 0 }} whileInView={{ scaleY: 1 }} viewport={{ once: true }} transition={{ duration: reduce ? 0 : 1.1, ease }} className="absolute bottom-0 left-[5px] top-[22px] w-px sm:top-6 origin-top bg-primary sm:left-[9px]" />
          {journey.map(([year, title, text], index) => <motion.article key={year} initial={reduce ? false : { opacity: 0 }} whileInView={{ opacity: 1 }} onViewportEnter={() => setActiveJourney(index)} viewport={{ once: true, amount: 0.55 }} transition={{ duration: 0.65, ease }} className={`relative min-h-40 border-b border-borderSoft pb-9 pt-1 last:border-0 sm:min-h-44 lg:min-h-[168px] ${activeJourney === index ? "opacity-100" : "opacity-60"}`}><span aria-hidden="true" className={`absolute -left-[26.5px] top-[22px] h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full border transition-[background-color,border-color,box-shadow] duration-300 motion-reduce:transition-none sm:-left-[38.5px] sm:top-6 ${activeJourney === index ? "border-primary bg-primaryGlow shadow-[0_0_16px_rgb(var(--color-primary-glow)/0.85)]" : "border-primary/45 bg-bg"}`} /><p className="font-display text-3xl text-primary sm:text-4xl">{year}</p><h3 className="mt-3 font-display text-xl sm:text-2xl">{title}</h3><p className="mt-3 max-w-2xl leading-relaxed text-textBody">{text}</p></motion.article>)}
        </div></div>
    </motion.section>
    ...
    <motion.section {...sectionMotion} className="section-shell border-t border-borderSoft" aria-labelledby="curiosity-title">
      ...
      <div className="mt-8 space-y-3 lg:hidden">
        {curiosities.map(([title, description], index) => (
          <details key={title} name="mobile-curiosities" className="group">
            <summary className="flex w-full cursor-pointer list-none items-center gap-4 rounded-xl border border-transparent px-5 py-4 text-left text-textDim transition-[color,background-color,border-color] duration-300 hover:bg-primary/5 hover:text-textMain group-open:border-primary/25 group-open:bg-primary/10 group-open:text-primary motion-reduce:transition-none [&::-webkit-details-marker]:hidden">
              <span className="text-xs font-semibold tracking-[0.12em] text-primary/80">{String(index + 1).padStart(2, "0")}</span>
              <span className="font-display text-lg sm:text-xl">{title}</span>
              <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" className="ml-auto h-4 w-4 shrink-0 opacity-55 transition-transform duration-300 group-open:rotate-90 motion-reduce:transition-none"><path d="m9 5 7 7-7 7" /></svg>
            </summary>
            <p className="mx-2 mt-2 rounded-2xl bg-gradient-to-br from-bgCard/70 to-primary/10 px-5 py-5 leading-[1.7] text-textBody sm:px-6">{description}</p>
          </details>
        ))}
      </div>
    </motion.section>
