...
export function AboutPage() {
  ...
  return <main id="main-content">
    <section className="section-shell !max-w-[1280px] !pb-16 pt-28 md:!pb-24 md:pt-36" aria-labelledby="about-title">
      <div className="grid gap-12 lg:grid-cols-[38fr_62fr] lg:items-center lg:gap-16 xl:gap-20">
        <motion.div initial={reduce ? false : { opacity: 0, y: 24, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.75, delay: 0.42, ease }} className="relative mx-auto aspect-[4/5] w-full max-w-[280px] overflow-hidden rounded-[45%_45%_10%_10%] border border-primary/25 bg-bgCard shadow-[0_16px_40px_rgb(20_54_35_/_0.06)] sm:max-w-[340px] md:max-w-[360px]"><Image src={portrait} alt="Portrait of Khin Myat Thu" fill priority className="object-cover object-[center_30%]" sizes="(max-width: 639px) 280px, (max-width: 1023px) 360px, 360px" /></motion.div>
        <div className="min-w-0">
          <motion.p initial={reduce ? false : { opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="eyebrow">About me</motion.p>
          <h1 id="about-title" className="mt-5 max-w-[700px] font-display text-4xl font-medium leading-[1.04] tracking-[-0.035em] sm:text-[2.625rem] md:text-5xl lg:text-[3.25rem] xl:text-[3.5rem]"><span className="block">Learning by doing.</span><span className="block italic text-primary">Building what matters.</span></h1>
          <motion.div initial={reveal} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.68, delay: 0.2, ease }} className="mt-7 max-w-[700px] space-y-5 text-base leading-[1.7] text-textMain/75 lg:text-[1.0625rem]"><p>I’m Khin Myat Thu, a final-year Computer Science student who learns best by turning ideas into working products. I enjoy exploring full-stack development, practical AI, and digital solutions that respond to real-world problems.</p><p>I’m always curious to learn something new, experiment with different technologies, and turn what I learn into something useful. For me, every project is an opportunity to grow, solve problems, and understand how technology can make a meaningful difference.</p></motion.div>
        </div>
      </div>
      <motion.div initial={reveal} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.32, ease }} className="mt-14 border-t border-borderSoft pt-9 md:mt-16 md:pt-10 lg:mt-20" aria-labelledby="education-title"><p id="education-title" className="eyebrow">Education</p><div className="mt-5 max-w-3xl"><div className="pb-7"><p className="font-display text-xl sm:text-2xl">B.E. (Hons) in Computer Science Engineering</p><p className="mt-2 text-sm text-textMuted sm:text-base">Myanmar Institute of Information Technology</p><p className="mt-1.5 text-sm font-medium text-primary">2022–2027</p></div><div className="border-t border-borderSoft py-7"><p className="font-display text-xl sm:text-2xl">Diploma in English</p><p className="mt-2 text-sm text-textMuted sm:text-base">Mandalay University of Foreign Languages</p><p className="mt-1.5 text-sm font-medium text-primary">2023–2024</p></div></div></motion.div>
    </section>
